...
CAN::CAN(CAN_HandleTypeDef* hcan, uint32_t start_id, bool is_master)
  ...
  */
  // commented due to a lack of ASSERT 2021.10.17 youhy
...
int CAN::Transmit(uint16_t id, const uint8_t data[], uint32_t length) {
  ...
  // we dont have expect() yet. 2021.10.17 youhy
  if (!IS_CAN_DLC(length) || HAL_CAN_IsTxMessagePending(hcan_, mailbox)) return -1;
