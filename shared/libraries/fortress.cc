...
Fortress::~Fortress() {
  for (int i = 0; i < FORTRESS_MOTOR_NUM; ++i) motors_[i] = nullptr;
  delete[] motors_;
  motors_ = nullptr;
}
...
void Fortress::Up() {
  for (int i = 0; i < FORTRESS_MOTOR_NUM; ++i) {
    motors_[i]->SetOutput(
        pids_[i].ComputeConstrainedOutput(motors_[i]->GetOmegaDelta(-FORTRESS_RISE_SPEED)));
  }
}
...
void Fortress::Rotate() {
  yaw_motor_->SetOutput(
      (yaw_pid_.ComputeConstrainedOutput(yaw_motor_->GetOmegaDelta(FORTRESS_ROTATE_SPEED))));
}
