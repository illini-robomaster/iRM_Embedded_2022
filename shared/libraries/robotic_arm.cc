...
RoboticArm::RoboticArm(robotic_arm_t robotic_arm) {
  ...
  servo_data[left].max_speed = 2 * PI;
  servo_data[left].max_acceleration = 2 * PI;
  servo_data[left].transmission_ratio = M3508P19_RATIO;
  servo_data[left].omega_pid_param = new float[3]{25, 5, 35};

  servo_data[right].motor = robotic_arm.motors[right];
  servo_data[right].mode = control::SERVO_NEAREST;
  servo_data[right].max_speed = 2 * PI;
  servo_data[right].max_acceleration = 2 * PI;
  servo_data[right].transmission_ratio = M3508P19_RATIO;
  servo_data[right].omega_pid_param = new float[3]{25, 5, 35};

  servo_data[gripper].motor = robotic_arm.motors[gripper];
  servo_data[gripper].mode = control::SERVO_NEAREST;
  servo_data[gripper].max_speed = 2 * PI;
  servo_data[gripper].max_acceleration = 2 * PI;
  servo_data[gripper].transmission_ratio = M3508P19_RATIO;
  servo_data[gripper].omega_pid_param = new float[3]{25, 5, 35};
...
void RoboticArm::SetPosition(const float arm_position, const float gripper_position) {
  servo_[left]->SetTarget(arm_position, true);
  servo_[right]->SetTarget(-1 * arm_position, true);
  servo_[gripper]->SetTarget(gripper_position, true);
}
