...
void RM_RTOS_Init() {
  ...
  left_fly_motor = new control::Motor3508(can1, 0x202);
  right_fly_motor = new control::Motor3508(can1, 0x201);
  load_motor = new control::Motor3508(can2, 0x205);
  ...
  shooter_data.model = control::SHOOTER_HERO_2022;
...
void RM_RTOS_Default_Task(const void* args) {
  UNUSED(args);
  control::MotorCANBase* can1_motor[] = {left_fly_motor, right_fly_motor};
  control::MotorCANBase* can2_motor[] = {load_motor};
  ...
  while (true) {
    ...
    gimbal->Update();
    shooter->Update();
    control::MotorCANBase::TransmitOutput(can1_motor, 2);
    control::MotorCANBase::TransmitOutput(can2_motor, 1);
    osDelay(10);
  }
}
